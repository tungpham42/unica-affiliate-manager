...
function uwaff_settings_page() {
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('Unica Affiliate Settings', 'unica-affiliate-manager'); ?></h1>
...
function uwaff_render_manual_import_form() {
    ...
    ?>
    <h2><?php esc_html_e('Manual Courses Import', 'unica-affiliate-manager'); ?></h2>
    <form method="post">
...
function uwaff_render_set_current_page_form() {
    ...
    ?>
    <h2><?php esc_html_e('Set Current Page for Import (each page has at most 15 courses)', 'unica-affiliate-manager'); ?></h2>
    <form method="post">
        <?php wp_nonce_field('set_current_page_action', 'set_current_page_nonce'); ?>
        <label for="uwaff_current_page"><?php esc_html_e('Current Page (start from 1):', 'unica-affiliate-manager'); ?></label>
        <input type="number" name="uwaff_current_page" id="uwaff_current_page" value="<?php echo esc_attr($displayed_page_index); ?>" min="1">
        <?php submit_button(esc_html__('Set Page', 'unica-affiliate-manager'), 'primary', 'set_current_page'); ?>
    </form>
    <?php
}
...
function uwaff_handle_manual_product_import() {
    if (isset($_POST['uwaff_manual_import'])) {
        ...
        if (wp_verify_nonce($nonce, 'import_products_action')) {
            ...
            echo '<p>' . esc_html__('Nonce verification failed.', 'unica-affiliate-manager') . '</p>';
        }
    }
}
...
function uwaff_handle_set_current_page() {
    if (isset($_POST['set_current_page'])) {
        ...
        if (wp_verify_nonce($nonce, 'set_current_page_action')) {
            ...
            echo '<p>' . esc_html__('Current page updated to ', 'unica-affiliate-manager') . esc_html($new_page + 1) . '</p>';
        } else {
            echo '<p>' . esc_html__('Nonce verification failed.', 'unica-affiliate-manager') . '</p>';
        }
    }
